...
# Далее параметры конфигуратора
  ...
  def self.open_ib_enterprise(common_params,thin_client=false)
    FileUtils.rm(common_params[:Out]) if File.exists?(common_params[:Out])
    FileUtils.rm(DUMPRES_FILE_DEF) if File.exists?(DUMPRES_FILE_DEF)
    system "\"#{ones_v8exe(thin_client)}\" ENTERPRISE /DumpResult\"#{DUMPRES_FILE_DEF}\" #{common_param_to_s(common_params)} "
    if $?.exitstatus != 0 then
  ...
  def self.common_param_to_s(hash)
    ...
    hash.each(){|key,value|
      next if key == :C
      result = param_to_s(result,key,value)
     }
    if hash.has_key?(:C)
      result = param_to_s(result,:C,hash[:C])
    end
     result
  end

  def self.param_to_s(result,key,value)
