...
Процедура СкопироватьНастройки(ИмяКоманды)
	...
	Если ПереключательКопируемыеНастройки = "СкопироватьОтдельные" Тогда
		...
		Если Отчет <> Неопределено Тогда
			...
			ПоказатьВопрос(ТекстВопроса, КнопкиВопроса,, КнопкиВопроса[0].Значение);
			Возврат;
		КонецЕсли;
	...
	Если РезультатПроверки = "ТекущийПользовательПолучатель" Или РезультатПроверки = "ТекущийПользовательСредиПолучателей" Тогда
		...
		Если ПереключательКопируемыеНастройки = "СкопироватьВсе" Тогда
			...
			ПредложениеПерезапускаПрограммы(ИмяКоманды);
...
&НаКлиенте
Функция ПредложениеПерезапускаПрограммы(ИмяКоманды)
	...
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Перезапустить", НСтр("ru='Перезапустить сейчас'"));
	КнопкиВопроса.Добавить("НеПерезапускать", НСтр("ru='Не перезапускать'"));
	Оповещение = Новый ОписаниеОповещения("ПредложениеПерезапускаПрограммыЗавершение", ЭтотОбъект, ИмяКоманды);
	ПоказатьВопрос(Оповещение, ТекстСообщения, КнопкиВопроса,, КнопкиВопроса[1].Значение);
	
КонецФункции

&НаКлиенте
Процедура ПредложениеПерезапускаПрограммыЗавершение(Ответ, ИмяКоманды) Экспорт
	
	Если Ответ = "Перезапустить" Тогда
		ПодключитьОбработчикОжидания("Подключаемый_ПерезапуститьПрограмму", 0.01, Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПерезапуститьПрограмму()
