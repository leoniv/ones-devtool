...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено, Знач АдресРезультата = "") Экспорт
	...
	Если Параметры <> Неопределено Тогда
		...
		// правильно ПривилегированнаяЗапись
	...
	ТаблицаПоиска.Колонки.Добавить("ВерсияДанных", ТипСтрока);
	...
	Для Каждого ТекущаяСсылка Из СписокСсылок Цикл
		...
		Иначе
			...
			Для Каждого Ошибка Из РезультатРазметки.ОшибкиРазметки Цикл
				ДобавитьРезультатЗамены(РезультатЗамены, ТекущаяСсылка, "НеизвестныеДанные", Ошибка.Объект, Ошибка.Текст);
			КонецЦикла;
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого Строка Из МестаИспользования Цикл
		...
		ИначеЕсли ТипВсеСсылки.СодержитТип(ТипДанных) Тогда
			МетаПредставление = Мета.ПредставлениеОбъекта;
			Если ПустаяСтрока(МетаПредставление) Тогда
...
Функция РаспределитьСуммуПропорциональноКоэффициентам(Знач ИсхСумма, МассивКоэф, Знач Точность = 2) Экспорт
	...
	Возврат ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(ИсхСумма, МассивКоэф, Точность);
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("МинимальноНеобходимаяВерсияПлатформы", "8.3.3.715");
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных
...
Функция ОбработчикиСобытия(Событие) Экспорт
	...
	Для каждого Обработчик Из Обработчики Цикл
		...
		Модуль = ОбщийМодуль(Обработчик.Модуль);
...
Функция ВнутренняяWSПрокси(Знач АдресWSDL, Знач URIПространстваИмен, Знач ИмяСервиса,
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернетаКлиентСервер") Тогда
		МодульПолучениеФайловИзИнтернетаКлиентСервер = ОбщийМодуль("ПолучениеФайловИзИнтернета");
	...
	Для Каждого МестоИспользования Из РезультатРазметки.МестаИспользования Цикл
		...
		Иначе
			...
			Если ТипВсеСсылки.СодержитТип(ТипДанных) Тогда
				...
				МестоИспользования.ВерсияДанных = МестоИспользования.Данные.ВерсияДанных;
				МестоИспользования.ЦелеваяСсылка = ЦелеваяСсылка;
...
Процедура ПроизвестиЗаменуВКонстанте(Результаты, Знач МестоИспользования, Знач ПараметрыЗаписи, Знач ВнутренняяТранзакция = Истина)
	...
	Если ВнутренняяТранзакция Тогда
		...
		Исключение
			...
			Ошибка = СтрЗаменить(Ошибка, "%1", Строка(Данные));
	...
	Если СостояниеОперации = "" Тогда
		...
		Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
			Если Менеджер.Значение = Строка.Ссылка Тогда
				...
				ЗаменаНеПроизведена = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ЗаменаНеПроизведена Тогда
			// Добавляем запись в результат о том, что объект был изменен
			Ошибка = НСтр("ru = 'Объект %1 был изменен другим пользователем'");
			Ошибка = СтрЗаменить(Ошибка, "%1", Строка(Данные));
			Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
				ДобавитьРезультатЗамены(Результаты, Строка.Ссылка, "ДанныеИзменены", Данные, Ошибка);
			КонецЦикла;
		...
		Иначе
			...
			Исключение
				...
				Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
					ДобавитьРезультатЗамены(Результаты, Строка.Ссылка, "ОшибкаЗаписи", Данные, Ошибка);
				КонецЦикла;
...
Процедура ПроизвестиЗаменуВОбъекте(Результаты, Знач МестоИспользования, Знач ПараметрыЗаписи, Знач ВнутренняяТранзакция = Истина)
	...
	Если ВнутренняяТранзакция Тогда
		...
		Исключение
			...
			Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
				ДобавитьРезультатЗамены(Результаты, Строка.Ссылка, "ОшибкаБлокировки", Данные, Ошибка);
			КонецЦикла;
	...
	Если СостояниеОперации = "" Тогда
		// Если объект был изменен, то считаем, что замена выполнена успешна
		
		Попытка
			ТекущаяВерсия = Данные.ВерсияДанных;
		Исключение
			ТекущаяВерсия = МестоИспользования.ВерсияДанных;
		КонецПопытки;
		
		Если ТекущаяВерсия <> МестоИспользования.ВерсияДанных Тогда
			// Добавляем запись в результат о том, что объект был изменен
			Ошибка = НСтр("ru = 'Объект %1 был изменен другим пользователем'");
			Ошибка = СтрЗаменить(Ошибка, "%1", Строка(Данные));
			Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
...
Процедура УдалитьСсылкиПометкой(РезультатУдаления, Знач СписокСсылок, Знач ПараметрыЗаписи, Знач ВнутренняяТранзакция = Истина)
	...
	Если ВнутренняяТранзакция Тогда
		...
		Для Каждого Ссылка Из СписокСсылок Цикл
			...
			Исключение
				ДобавитьРезультатЗамены(РезультатУдаления, Ссылка, "ОшибкаБлокировки", Ссылка, 
					НСтр("ru = 'Ошибка удаления ссылки, она используется другим пользователем'"));
				Продолжить;
			КонецПопытки
	...
	Для Каждого Ссылка Из Удаляемые Цикл
		...
		Если МестаИспользования.Количество()=0 Тогда
			...
			Попытка
				УстановитьПометкуУдаления(Объект, ПараметрыЗаписи);
			Исключение
				ДобавитьРезультатЗамены(РезультатУдаления, Ссылка, "ОшибкаЗаписи", Ссылка, 
					НСтр("ru = 'Ошибка пометки удаления'"));
			КонецПопытки;
...
Процедура УдалитьСсылкиНепосредственно(РезультатУдаления, СписокСсылок, ПараметрыЗаписи, ВнутренняяТранзакция)
	...
	Если ПараметрыЗаписи.ПривилегированнаяЗапись Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	Попытка
		УдалитьОбъекты(СписокСсылок, Истина, МестаИспользования);
		ДобавитьРезультатыЗаменыИзмененныхОбъектов(РезультатУдаления, МестаИспользования);
	Исключение
		СостояниеТранзакции = "ОшибкаУдаления";
		
		ДобавитьРезультатЗамены(РезультатУдаления, Неопределено, СостояниеТранзакции, Неопределено, 
			НСтр("ru = 'Ошибка безвозвратного удаления объектов'"));
	КонецПопытки;
	...
	Если ВнутренняяТранзакция Тогда
		Если СостояниеТранзакции = "" Тогда
			ЗафиксироватьТранзакцию();
		Иначе
			ОтменитьТранзакцию();
		КонецЕсли;
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(ИтоговаяТаблица, ТаблицаПовторногоПоиска)
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		Если ИтоговаяТаблица.НайтиСтроки(Фильтр).Количество() = 0 Тогда
			ДобавитьРезультатЗамены(ИтоговаяТаблица, Ссылка, "ДанныеИзменены", Данные, 
...
Функция ОписаниеИзмеренийНабора(Знач Мета, Кэш)
	...
	Если Метаданные.РегистрыСведений.Содержит(Мета) Тогда
		...
		ИначеЕсли МетаПериод = Периодичность.Секунда Тогда
			ДанныеИзмерения = Новый Структура;
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("ru='Период'");
...
Функция ОписаниеПоследовательностей(Знач Мета)
	...
	Для Каждого Последовательность Из Метаданные.Последовательности Цикл
		...
		ОписаниеПоследовательностей.Вставить("НаборЗаписей", Последовательности[Последовательность.Имя].СоздатьНаборЗаписей());
		ОписаниеПоследовательностей.Вставить("ПространствоБлокировки", ИмяТаблицы + ".Записи");
		ОписаниеПоследовательностей.Вставить("Измерения", Новый Структура);
		
	КонецЦикла;	// Последовательность
...
Процедура ДобавитьРезультатЗамены(Таблица, Знач Ссылка, Знач ТипОшибки = "", Знач ОбъектОшибки = Неопределено, Знач ТекстОшибки = "")
	Строка = Таблица.Добавить();
	Строка.Ссылка       = Ссылка;
	Строка.ТипОшибки    = ТипОшибки;
	Строка.ОбъектОшибки = ОбъектОшибки;
	Строка.ТекстОшибки  = ТекстОшибки;
	
	Строка.ПредставлениеОбъектаОшибки = Строка(ОбъектОшибки);
КонецПроцедуры
...
Функция ПолучитьТекущиеСообщенияПользователя()
	Результат = Новый Массив;
...
Процедура ЗаписатьОбъектСПерехватомСообщений(Знач Объект, Знач РежимЗаписи = Неопределено)
	
	// Текущие сообщения до исключения запоминаем
	ПредыдущиеСообщения = ПолучитьТекущиеСообщенияПользователя();
	...
	Исключение
		...
		// Перехватываем все сообщенное при ошибке и преобразуем их в одно исключение
		ТекстИсключения = "";
		Для Каждого Сообщение Из ПолучитьТекущиеСообщенияПользователя() Цикл
		...
		ВызватьИсключение СокрЛП(КраткоеПредставлениеОшибки(Информация) + Символы.ПС + ТекстИсключения);
	КонецПопытки;
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Для Каждого Тип Из Документы.ТипВсеСсылки().Типы() Цикл
		...
		ТипДокументОбъект = Тип("ДокументОбъект." + Сред(ИмяТипа, 1 + Найти(ИмяТипа, ".")) );
		Если ТипОбъекта = ТипДокументОбъект Тогда
			// Это документ
			ЗаписатьОбъектСПерехватомСообщений(Объект, РежимЗаписиДокумента.Запись);
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	ЗаполнитьЗначенияСвойств(ТестРеквизитов, Объект.Метаданные());
	...
	// Просто запись
	ЗаписатьОбъектСПерехватомСообщений(Объект);
КонецПроцедуры

Процедура УстановитьПометкуУдаления(Объект, Знач ПараметрыЗаписи)
...
Процедура ЗаменитьСсылкуДлиннойТранзакцией(РезультатЗамены, Знач Ссылка, Знач ПараметрыЗамены, Знач ТаблицаПоиска)
	...
	Исключение
		...
		ДобавитьРезультатЗамены(РезультатЗамены, Ссылка, "ОшибкаБлокировки", Неопределено, Ошибка);
